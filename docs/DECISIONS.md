...
### D8 — PII exemptions are inline; the allowlist holds generated files only

Synthetic PII-shaped literals in hand-written code carry a `pii-allow` marker on
their own line rather than the directory being blanket-exempted, so the exemption
is visible exactly where it applies.

**Updated in Phase 8.** The allowlist was empty until `web/package-lock.json`
tripped the PAN pattern: npm integrity hashes are base64 and some segments read
as five uppercase letters, four digits and an uppercase letter. A lockfile cannot
contain patient data, and it is regenerated by every `npm install` so an inline
marker could not survive there. Machine-generated files are the only thing the
allowlist is for, and each entry carries its reason.
