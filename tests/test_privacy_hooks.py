...
def test_allowlist_contains_only_generated_files() -> None:
    """No hand-written source or test path may be blanket-exempt.

    This replaces an earlier assertion that the allowlist was EMPTY, which held
    until npm's package-lock.json tripped the PAN pattern on a base64 integrity
    hash. A lockfile is regenerated by every install, so an inline marker cannot
    survive in it - but the exemption must stay narrow, and the property that
    actually matters is that nothing a human writes is exempt.
    """
    from tools.hooks.pii_scan import load_allowlist

    allowed = load_allowlist(REPO / "tools" / "pii_allowlist.txt")
    generated_suffixes = ("package-lock.json", "uv.lock", "yarn.lock",
                          "pnpm-lock.yaml")
    for entry in allowed:
        assert entry.endswith(generated_suffixes), (
            f"{entry} is not a generated lockfile; exempt it inline instead"
        )
        assert not entry.startswith(("reckon/", "tests/", "tools/", "scripts/")), (
            f"{entry} is hand-written source and must not be blanket-exempt"
        )


def test_source_and_tests_are_never_allowlisted() -> None:
    """The directories where a real leak would actually land stay scanned."""
    from tools.hooks.pii_scan import is_allowlisted, load_allowlist

    allowed = load_allowlist(REPO / "tools" / "pii_allowlist.txt")
    for path in ("reckon/schema.py", "tests/test_schema.py",
                 "tools/hooks/pii_scan.py", "data/real/bill.txt"):
        assert not is_allowlisted(path, allowed), path
